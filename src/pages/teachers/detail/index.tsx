...
const TAB_CONFIG: Record<string, { label: string; addLabel: string; modalType: string }> = {
  researches: { label: "Tadqiqotlar", addLabel: "Tadqiqot qo'shish", modalType: "research" },
  publications: { label: "Nashrlar", addLabel: "Nashr qo'shish", modalType: "publication" },
  supervision: { label: "Nazoratlar", addLabel: "Nazorat qo'shish", modalType: "nazorat" },
  activities: { label: "Maslahatlar", addLabel: "Maslahat qo'shish", modalType: "maslahat" },
  awards: { label: "Mukofotlar", addLabel: "Mukofot qo'shish", modalType: "mukofot" },
};
...
export default function TeacherDetail() {
  ...
  if (!teacher) return <div className="p-10 text-center text-muted-foreground">O'qituvchi topilmadi.</div>;
  ...
  const profileValues: ProfileFormData = {
    ...
    age: teacher.age ?? "",
    phone: teacher.phone ?? "",
    department: departmentsOptions.find((d) => d.label === teacher.departmentName)?.value ?? "",
    position: positionsOptions.find((l) => l.label === teacher.lavozimName)?.value ?? "",
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      <nav className="flex items-center gap-2 text-[13px] text-muted-foreground">
        <button className="hover:text-foreground transition-colors" onClick={() => navigate("/teachers")}>
          O'qituvchilar
        </button>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="mt-4">
          <StatsGrid 
             stats={stats} 
             subStats={{ international: 0, articles: 0, books: 0, supervised: 0, conferences: 0 }} 
          />
        </div>
      </div>
